...
class protocol_4:
    ...
    #During onliste testing, nacelle direction isn't updated as in here, instead it's picked up via Rotary Encoder.
        #Yawing doesn't use a PID control as the yawing system + nacelle has a lot of dampin and it stops right after we stop it, so it's simpler to control it like this!
        #sleep timeis exxagerated to (5*t) so that it stays for a while, as the changes in wind direction is very less, hence the time SMALL
    ...
    # Check if wind is blowing from the shadow sector
        #Shadow sector is the region where the wind turbine should never turn into. Else the yawing motor will collide with the wings.
        #Its a NO NO region.
    def is_Shadow(self):
